...
PokemonList readPokemonCSV(const char *filePath) {
    ...
    pokemonList.size = 0;
    pokemonList.pokemons = malloc(sizeof(Pokemon) * 1000); // Aloca��o inicial

    char line[1024];
    fgets(line, sizeof(line), file); // Pular o cabe�alho
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        token = strtok(NULL, ",");
        pokemon.weight = atoi(token);

        token = strtok(NULL, ",");
        pokemon.height = atoi(token);
